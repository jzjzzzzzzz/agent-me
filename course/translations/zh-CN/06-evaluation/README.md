...
运行评估需要非空且可读取的知识库。目录不存在或为空时报告 `knowledge_corpus_empty`；其他加载失败保留稳定错误码，例如将文件用作目录时的 `knowledge_directory_invalid`。这些配置错误退出码为 `2`，仅向 stderr 输出诊断，不向 stdout 输出通过或失败的评估摘要。此行为适用于两个 workflow 以及文本、JSON、Markdown 输出。`--list` 只校验并列出 fixture ID，不加载知识文件。知识库有效时，全部预期通过仍退出 `0`，行为与标签不一致仍退出 `1`。
