...
完整示例见[基于证据的 verified 策略 Capstone](../../../examples/verified-policy-capstone/README.zh-CN.md)，其中包含可复现命令、否决方案、实测输出、隐私检查、限制与诚实的作品集表述。
...
完成协作运行后，前端可下载只包含 `run_id`、`workflow`、`mode`、`answer`、`grounded`、
`sources` 与 `trace` 的 UTF-8 JSON。它明确排除提交的问题、个人资料元数据、模型服务配置
和浏览器隐藏状态。该文件只能作为 commit、命令、测试和限制说明的辅助证据，不能取代
这些材料，也不能证明生产质量。
