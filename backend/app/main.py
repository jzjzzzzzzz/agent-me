...
@app.get("/ready")
async def ready(config: Settings = Depends(get_settings)) -> dict[str, object]:
    documents = KnowledgeBase(config.knowledge_dir).documents()
    return {"status": "ready", "knowledge_documents": len(documents)}
...
async def chat(
    payload: ChatRequest, config: Settings = Depends(get_settings)
) -> ChatResponse:
